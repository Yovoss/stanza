...
from stanza.models.common.doc import SENTIMENT
...
def log_config(config_name, config):
    """
    Output the model's config
    Useful for debugging what the model is being used for
    """
    logger.debug("-- %s --" % config_name)
    for k in config.__dict__:
        logger.debug("  --{}: {}".format(k, config.__dict__[k]))

def log_labels(labels):
    logger.debug("-- MODEL LABELS --")
    logger.debug("  {}".format(" ".join(labels)))
...
def load(filename, pretrain):
    ...
    log_labels(checkpoint['labels'])
    log_config("SAVED CONFIG", checkpoint['config'])
...
def label_document(model, doc, reverse_label_map=None, device=None):
    """
    Given a pipeline Document object, extract the text, run the model, attach the results.
    """
    model.eval()
    ...
    # TODO: process text here!  needs to be split when sent to the model?
    # eg, a call to update_text
    output = model(text, device)
    for i in range(len(text)):
        predicted = torch.argmax(output[i])
        labels.append(reverse_label_map[predicted.item()])
    # TODO: allow a classifier model to be loaded for any attribute, not just sentiment
    doc.set(SENTIMENT, labels, to_sentence=True)

    logger.debug("Found sentiment labels")
...
def main():
    ...
    logger.info("Training set has %d labels" % len(labels))
